...
- Template placeholder replacement in `create-repo-with-plan-docs.ps1` must be verified — it currently replaces `workflow-orchestration-queue-hotel41-b` in file contents; the consumer `devcontainer.json` image URL references `workflow-orchestration-prebuild` so it should be unaffected, but this needs confirmation.
